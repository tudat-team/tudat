...
getInitialStatesOfBodiesFromFrameManager(
    const std::vector<std::string> &bodiesToIntegrate,
    const std::vector<std::string> &centralBodies,
    const simulation_setup::SystemOfBodies &bodies, const TimeType initialTime,
  ...
  for (unsigned int i = 0; i < bodiesToIntegrate.size(); i++) {
    ...
    if (!ephemerisOfCurrentBody) {
      throw std::runtime_error(
          "Could not determine initial state for body " +
    ...
    systemInitialState.segment(i * 6, 6) =
        ephemerisOfCurrentBody
            ->getTemplatedStateFromEphemeris<StateScalarType, TimeType>(
                initialTime);

    // Correct initial state if integration origin and ephemeris origin are not
    // equal.
    if (centralBodies.at(i) !=
        ephemerisOfCurrentBody->getReferenceFrameOrigin()) {
      std::shared_ptr<ephemerides::Ephemeris> correctionEphemeris =
          frameManager->getEphemeris(
              ephemerisOfCurrentBody->getReferenceFrameOrigin(),
              centralBodies.at(i));
      systemInitialState.segment(i * 6, 6) -=
          correctionEphemeris
...
template <typename TimeType = double, typename StateScalarType = double>
Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1>
getInitialRotationalStatesOfBodies(
    const std::vector<std::string> &bodiesToIntegrate,
  ...
  Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1> systemInitialState =
      Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1>::Zero(
          bodiesToIntegrate.size() * 7, 1);
  std::shared_ptr<ephemerides::RotationalEphemeris> rotationModelOfCurrentBody;

  // Iterate over all bodies.
  for (unsigned int i = 0; i < bodiesToIntegrate.size(); i++) {
    rotationModelOfCurrentBody =
        bodies.at(bodiesToIntegrate.at(i))->getRotationalEphemeris();
    ...
    systemInitialState.segment(i * 7, 7) =
        rotationModelOfCurrentBody->getRotationStateVector(initialTime)
            .template cast<StateScalarType>();

    // Correct initial state if integration origin and rotation model origin are
    // not equal.
    if (baseOrientations.at(i) !=
        rotationModelOfCurrentBody->getBaseFrameOrientation()) {
      throw std::runtime_error(
          "Error, cannot get initial rotational state w.r.t. non-base frame");
    }
  }
  return systemInitialState;
}
...
Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1> getInitialStateOfBody(
    const std::string &bodyToIntegrate, const std::string &centralBody,
    const simulation_setup::SystemOfBodies &bodies,
    const TimeType initialTime) {
...
template <typename TimeType = double, typename StateScalarType = double>
Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1>
getInitialRotationalStateOfBody(const std::string &bodyToIntegrate,
                                const std::string &baseOrientation,
                                const simulation_setup::SystemOfBodies &bodies,
                                const TimeType initialTime) {
  return getInitialRotationalStatesOfBodies<TimeType, StateScalarType>(
      std::vector<std::string>{bodyToIntegrate},
      std::vector<std::string>{baseOrientation}, bodies, initialTime);
}
...
Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1> getInitialArcWiseStateOfBody(
  ...
  for (unsigned int i = 0; i < arcStartTimes.size(); i++) {
    initialStates.block(6 * i, 0, 6, 1) =
        getInitialStateOfBody<double, StateScalarType>(
            bodyToIntegrate, centralBodies.at(i), bodies, arcStartTimes.at(i));
  }
  return initialStates;
}

////! Function to get a vector of initial states from a vector of propagator
/// settings
///*!
// *  Function to get a vector of initial states from a vector of propagator
// settings.
...
//        getInitialStatesPerArc(
//                const
//                std::vector<std::shared_ptr<PropagatorSettings<StateScalarType>
//                > > propagatorSettings) {
//        std::vector<Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1> >
//        initialStatesList; for (unsigned int i = 0; i <
//        propagatorSettings.size(); i++) {
...
//}
//
////! Function to get the initial state of a translational state arc from the
/// previous state's numerical solution
///*!
// *  Function to get the initial state of a translational state arc from the
// previous state's numerical solution
...
//        Eigen::Dynamic, 1> > &previousArcDynamicsSolution, const double
//        currentArcInitialTime) {
//    Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1> currentArcInitialState;
//    {
//        // Check if overlap exists
//        if (previousArcDynamicsSolution.rbegin()->first <
//        currentArcInitialTime) {
//            throw std::runtime_error(
//                    "Error when getting initial arc state from previous arc:
//                    no arc overlap");
//        } else {
//            int currentIndex = 0;
//            int initialTimeIndex = -1;
//
//            std::map<TimeType, Eigen::Matrix<StateScalarType, Eigen::Dynamic,
//            1> > initialStateInterpolationMap;
//
//            // Set sub-part of previous arc to interpolate for current arc
//            for (typename std::map<TimeType, Eigen::Matrix<StateScalarType,
//            Eigen::Dynamic, 1> >:: const_reverse_iterator previousArcIterator
//            = previousArcDynamicsSolution.rbegin();
//                    previousArcIterator != previousArcDynamicsSolution.rend();
//                    previousArcIterator++) {
//                initialStateInterpolationMap[previousArcIterator->first] =
//                previousArcIterator->second; if (initialTimeIndex < 0) {
...
//                    TimeType, Eigen::Matrix<StateScalarType, Eigen::Dynamic,
//                    1>, long double> >( initialStateInterpolationMap,
//                    8)->interpolate(currentArcInitialTime);
//
//        }
//    }
