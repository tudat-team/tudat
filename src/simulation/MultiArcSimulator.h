...
class MultiArcDynamicsSimulator : public BaseSimulator<StateScalarType, TimeType> {
public:
    ...
    MultiArcDynamicsSimulator(
        ...
        if (multiArcPropagatorSettings_ == nullptr) {
            throw std::runtime_error(
                    "Error when creating multi-arc dynamics simulator, input is not multi arc");
        } else {
            ...
            if (singleArcSettings.size() != arcStartTimes.size()) {
                throw std::runtime_error(
                        "Error when creating multi-arc dynamics simulator, input is inconsistent");
            }
    ...
    BaseSimulator<StateScalarType, TimeType>(
        ...
        if (multiArcPropagatorSettings_ == nullptr) {
            ...
            if (singleArcSettings.size() != integratorSettings.size()) {
                throw std::runtime_error(
                        "Error when creating multi-arc dynamics simulator, input sizes are inconsistent");
            }
    ...
    void integrateEquationsOfMotion(
        ...
        for (unsigned int i = 0; i < singleArcDynamicsSimulators_.size(); i++) {
            ...
            propagationTerminationReasons_[i] = singleArcDynamicsSimulators_.at(
                    i)->getPropagationTerminationReason();
     ...
     */
    std::vector<std::shared_ptr<DynamicsStateDerivativeModel<TimeType, StateScalarType> > >
     ...
     */
    std::vector<std::shared_ptr<SingleArcDynamicsSimulator<StateScalarType, TimeType> > >
