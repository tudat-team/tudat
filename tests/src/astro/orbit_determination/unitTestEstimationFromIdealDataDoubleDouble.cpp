...
BOOST_AUTO_TEST_CASE( test_EstimationFromPosition )
{
...

    {
        ...
        std::pair<std::shared_ptr<simulation_setup::EstimationOutput<double> >,
            std::shared_ptr<simulation_setup::EstimationInput<double, double> > > podDataOutput;
        Eigen::VectorXd estimationError = tudat::unit_tests::executeEarthOrbiterParameterEstimation<double, double>(
            podDataOutput, 1.0E7, 3, 2, true, true );

        Eigen::VectorXd firstIterationUpdate = podDataOutput.first->parameterHistory_[ 1 ] - podDataOutput.first->parameterHistory_[ 0 ];
        Eigen::VectorXd secondIterationUpdate = podDataOutput.first->parameterHistory_[ 2 ] - podDataOutput.first->parameterHistory_[ 1 ];

        BOOST_CHECK_CLOSE_FRACTION(
            firstIterationUpdate( 1 ) / firstIterationUpdate( 0 ), -0.5, 1.0E-8 );
        BOOST_CHECK_CLOSE_FRACTION(
            firstIterationUpdate( 7 ) / firstIterationUpdate( 6 ), 0.2, 1.0E-8 );

        BOOST_CHECK_CLOSE_FRACTION(
            secondIterationUpdate( 1 ) / secondIterationUpdate( 0 ), -0.5, 1.0E-6 );
        BOOST_CHECK_CLOSE_FRACTION(
            secondIterationUpdate( 7 ) / secondIterationUpdate( 6 ), 0.2, 1.0E-6 );
